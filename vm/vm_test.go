...
func testBooleanObject(expected bool, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object has the wrong value, expected %t, got %t", expected, result.Value)
	}
	return nil
}
...
func testStringObject(expected string, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object has the wrong value, expected %q, got %q", expected, result.Value)
	}
	return nil
}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []vmTestCase{
		...
		{`len("one", "two")`,
			&object.Error{
				Message: "wrong number of arguments, want 1, got 2",
			},
		},
		{`len([1, 2, 3])`, 3},
		{`len([])`, 0},
		...
		{`first(1)`,
			&object.Error{
				Message: "argument to `first` must be of type ARRAY, got INTEGER",
			},
		},
		{`last([1, 2, 3])`, 3},
		{`last([])`, Null},
		{`last(1)`,
			&object.Error{
				Message: "argument to `last` must be of type ARRAY, got INTEGER",
			},
		},
		{`rest([1, 2, 3])`, []int{2, 3}},
		{`rest([])`, Null},
		...
		{`push(1, 1)`,
			&object.Error{
				Message: "argument to `push` must be of type ARRAY, got INTEGER",
			},
		},
	}
	runVmTests(t, tests)
